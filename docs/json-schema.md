...
| Field | Type | Required | Default | Description |
|-------|------|----------|---------|-------------|
| `src` | `string` | **Yes** | — | Non-empty image source path. Current renderer support is limited to local filesystem paths. Relative paths resolve under the configured asset root, and traversal outside that root is rejected. |
